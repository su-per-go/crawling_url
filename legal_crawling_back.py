...
def loop():
    root_dir = "e:/code/dataset/legal_back/"
    ...
    while True:
        ...
        if the_breakpoint["read_num"] > len(url_ls):
            break
        state_code, redirect, login_url = analyzer.analyze_page("https://" + url_ls[the_breakpoint["read_num"]] + "/",
                                                                root_dir,
                                                                the_breakpoint["save_num"])

        print("第" + str(the_breakpoint["save_num"]) + "个", state_code, the_breakpoint["read_num"], "普通界面")
        save_url_info(root_dir + "url_info.csv", "https://" + url_ls[the_breakpoint["read_num"]] + "/",
